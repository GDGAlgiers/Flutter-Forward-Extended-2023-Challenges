...
import '../../ui/detail_surah_screen.dart';
...
class LastReadCubit extends Cubit<LastReadState> {
  ...
  void getLastRead(void f) async {
    ...
    result.fold(
        ...
        (data) => emit(LastReadState(data: data, message: f as String)));
  }
  ...
  Future<void> addLastRead(DetailSurahEntity surah) async {
    ...
    result.fold(
        (failure) =>
            emit(LastReadState(data:  [], message: failure.message)),
  ...
  Future<void> updateLastRead(DetailSurahEntity surah) async {
    ...
    result.fold(
        (failure) =>
            emit(LastReadState(data: const [], message: failure.message)),
        (data) => emit(
            LastReadState(data: const [], message: LastReadCubit as String)));
  }
}
